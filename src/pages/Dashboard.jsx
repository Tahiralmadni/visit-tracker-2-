...
const countQuestionsInVisits = (visits, selectedMonth = null, officerName = null, selectedYear = null) => {
  ...
  const filteredVisits = visits.filter(visit => {
    ...
    // If selectedYear is provided, filter by year
    if (selectedYear && selectedYear !== 'all') {
      const visitYear = dayjs(visit.date).format('YYYY');
...
const countVisitsPerOfficer = (visits, officerName, selectedMonth = null, selectedYear = null) => {
  return visits.filter(visit => {
    ...
    if (selectedYear && selectedYear !== 'all') {
      const visitYear = dayjs(visit.date).format('YYYY');
      if (visitYear !== selectedYear) return false;
    }
...
const countTotalMinutes = (visits, officerName = null, selectedMonth = null, selectedYear = null) => {
...
function Dashboard({ visits = [], onDelete }) {
  ...
  useEffect(() => {
    ...
    if (isAdmin || isOfficer) {
      ...
      // Count questions in the current month/year using proper question counting
      const filteredVisits = visitsToCount.filter(v => {
        const vMonth = dayjs(v.date).format('MM');
        const vYear = dayjs(v.date).format('YYYY');
        const monthMatch = vMonth === displayMonth;
        const yearMatch = selectedYear === 'all' || vYear === selectedYear;
    ...
    setIsLoading(false);
  }, [visits, isOfficer, officerName, isAdmin, displayMonth, currentMonth, selectedYear, userRole]);
  ...
  const getFilteredVisits = () => {
    const filteredResults = localVisits.filter(visit => {
      ...
      // Filter by Year
      if (selectedYear !== 'all' && visitYear !== selectedYear) {
         return false;
      }
  ...
  }, [localVisits, searchCategory, searchQuery, selectedMonths, searchDate, userRole, displayMonth, selectedYear]);

  // Compute total monthly questions dynamically to avoid render loop lags
  const totalMonthlyQuestions = useMemo(() => {
    if (searchCategory === 'officerName' && searchQuery) {
      return countQuestionsInVisits(
        filteredVisits.filter(v => dayjs(v.date).format('MM') === displayMonth),
        displayMonth,
        officerName,
        selectedYear
      );
  ...
  }, [filteredVisits, localVisits, searchCategory, searchQuery, displayMonth, selectedYear, officerName]);
                    ...
                    <CardContent sx={{ p: 3, position: 'relative', zIndex: 2 }}>
                      ...
                      <Typography variant="h2" component="div" sx={{ 
                        ...
                        {isLoading ? (
                          ...
                          formatNumber(countTotalMinutes(visits, isOfficer ? officerName : null, displayMonth, selectedYear), i18n.language)
                        )}
                      </Typography>
                    </CardContent>
                  </Card>
                  ...
                  <Box sx={{ 
                    ...
                    <Grid container spacing={3}>
                      {OFFICERS.map(officer => {
                        ...
                        const officerVisits = countVisitsPerOfficer(visits, officer, selectedMonthValue, selectedYear);
                        
                        // Count questions using the improved function that detects question numbers (n)
                        const officerQuestions = countQuestionsInVisits(visits, selectedMonthValue, officer, selectedYear);
                        
                        // Calculate total minutes for this officer
                        const totalMinutes = countTotalMinutes(visits, officer, selectedMonthValue, selectedYear);
